...
import RM4.Server.MTLServer;
import RM4.Server.QUEServer;
import RM4.Server.SHEServer;
import RM4.damsImplementation.MTL_Impl;
import RM4.damsInterface.DamsInterface;
import RM4.model.Message;
...
public class RM4 {
    public static String Bug_ID = "RM4_BUG";
    ...
    private static void receive() throws Exception{
        Runnable helloRunnable = new Runnable() {
            public void run() {
                if(!host_ip.isEmpty()){
                    messsageToFront("Heartbeat from RM4",host_ip);
                }
            }
        };
        ...
        try {
            ...
            System.out.println("RM4 UDP Server Listening for Multicast from Sequencer (port=1234)............");
            ...
            while (true) {
                ...
                Message Types:
                    ...
                    03-RM4 had bug and updating its list
                    11-RM4 has bug
                    21-RM4 is down
                */
                if(msg_type.equalsIgnoreCase("00")){
                    ...
                    Runnable crash_task = () -> {
                        try {
                            ...
                            DamsInterface mtlImpl = (DamsInterface) registry_mtl_patient_admin.lookup("MTL");
                            mtlImpl.shutDown();
                            System.out.println("RM4 shutdown Montreal Server");

                            registry_que_patient_admin = LocateRegistry.getRegistry(1000);
                            DamsInterface queImpl = (DamsInterface) registry_que_patient_admin.lookup("QUE");
                            queImpl.shutDown();
                            System.out.println("RM4 shutdown Quebec Server");

                            registry_she_patient_admin = LocateRegistry.getRegistry(1020);
                            DamsInterface sheImpl = (DamsInterface) registry_she_patient_admin.lookup("SHE");
                            sheImpl.shutDown();
                            System.out.println("RM4 shutdown Quebec Server");
    ...
    private static String requestToServers(Message input) throws Exception {
        startRegistry(7778);
        String URL = "";
        DamsInterface DamsInterface = null;
        if (input.patientID.startsWith("MTL")) {
            registry_mtl_patient_admin = LocateRegistry.getRegistry(1030);
            DamsInterface = (DamsInterface) registry_mtl_patient_admin.lookup("Mtl");
        } else if (input.Function.startsWith("QUE")) {
            registry_que_patient_admin = LocateRegistry.getRegistry(1040);
            DamsInterface = (DamsInterface) registry_que_patient_admin.lookup("Que");
        } else if (input.Function.startsWith("SHE")) {
            registry_she_patient_admin = LocateRegistry.getRegistry(1050);
            DamsInterface = (DamsInterface) registry_she_patient_admin.lookup("She");
        }
        ...
        if (input.patientID.charAt(3) == 'P') {
            if (input.Function.equalsIgnoreCase("BOOK_APPOINTMENT")) {
                ...
                String response = DamsInterface.GetAppointment(input.patientID);
                System.out.println(response);
                return response;
            } else if (input.Function.equalsIgnoreCase("CANCEL_APPOINTMENT")) {
                String response = DamsInterface.CancleAppointment(input.patientID, input.newAppointmentID);
                System.out.println(response);
                return response;
            ...
            if (input.Function.equalsIgnoreCase("BOOK_APPOINTMENT")) {
                ...
                String response = DamsInterface.ListAppointment(input.newAppointmentType);
                System.out.println(response);
                return response;
            }
        }
