...
public class MTL_Impl extends UnicastRemoteObject implements DamsInterface {
	public HashMap<String, HashMap<String, Mydetails>> database = null;
	DatagramSocket asocket = null;
	String resultString_mtl_que = "";
	String resultString_mtl_she ="";
	String apid;
	String Apt ;
	int slots;
	public MTL_Impl() throws RemoteException {
		super();
	...
	public String GetAppointment(String Uid) throws RemoteException {
		...
		for(Entry<String, HashMap<String, Mydetails>> data : database.entrySet()){
			HashMap<String, Mydetails> tmp = data.getValue();

			for(Entry<String, Mydetails> details : tmp.entrySet()){
	...
	public String AddAppointment(String apid, String Apt, int slots) throws RemoteException {
		...
		HashMap<String, Mydetails> tmplistHashMap = new HashMap<>();
		String result = "";
...
//				if (database.get(Apt).get(apid).equals()){
//					System.out.println("You have already booked appointment with this type...");
//				}else {
...
//				}
		...
		new Thread(() -> {
			try {
				...
				resultString_mtl_que = new String(reply.getData()).trim();
			} catch (Exception e1) {
				e1.printStackTrace();
			}
		}).start();
//SHERBROOK
		new Thread(() -> {
			try {
				...
				DatagramPacket reply = new DatagramPacket(buffer, buffer.length);
				asocket.receive(reply);
				String resultString_mtl_she = new String(reply.getData()).trim();
				System.out.println("SHE result : "+resultString_mtl_she);
	...
	public String CancleAppointment(String Uid, String apid) throws RemoteException {
		...
		for(Entry<String, HashMap<String, Mydetails>> entry : database.entrySet()){
			HashMap<String, Mydetails> tmpdata = entry.getValue();
			for(Entry<String, Mydetails> data : tmpdata.entrySet()){
	...
	public String RemoveAppointment(String apid, String Apt) throws RemoteException{
		...
		if(database.containsKey(Apt)){
			...
			result = "No Appointment Available for this type!";
		}
		return result;
	}
...
//SHERBROOK
		new Thread(() -> {
			try {
				...
				DatagramPacket reply = new DatagramPacket(buffer, buffer.length);
				asocket.receive(reply);
				resultString_mtl_she = new String(reply.getData()).trim();
				System.out.println("SHE result : "+resultString_mtl_she);
		...
		if (database.containsKey(Apt)) {
			for (Entry<String, Mydetails> data : database.get(Apt).entrySet()) {
				data.getKey();
		...
		String finalString = resultString+resultString_mtl_que+resultString_mtl_she;
...
//SHERBROOK
	...
	public String get_listappointmentdata(String Apt) {
		String resultString = "";
		System.out.println(Apt+"db");
		if (database.containsKey(Apt.trim())) {
			System.out.println(Apt);

			for (Entry<String, Mydetails> data : database.get(Apt.trim()).entrySet()) {
				data.getKey();

				if ((data.getValue().getSlots() - data.getValue().getUid().size()) > 0) {
					resultString += data.getKey() + ":" + (data.getValue().getSlots() - data.getValue().getUid().size())
							+ ",";
					System.out.println(resultString+"DHGFHDGF");
				}

			}
		}
		System.out.println(resultString+"idfhihfiwhfie");
		return resultString;
	}
